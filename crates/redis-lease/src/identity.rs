...
/// Token для проверки владения lease: ключ, владелец и номер захвата (fence).
///
/// При новом захвате Lua-скрипт увеличивает счётчик fence в Redis.
/// Поэтому после освобождения или истечения lease старый token
/// больше не подтверждает владение, даже если владелец тот же.
///
