...
impl PublishMessage {
  ...
  pub fn message_id(mut self, message_id: impl Into<String>) -> Result<Self, PublishMessageError> {
...
mod tests {
  ...
  fn sets_nats_message_id_header() {
    let (subject, message) = PublishMessage::new("events.messages", Bytes::from_static(b"payload"))
      .expect("publish message must be valid")
  ...
  fn rejects_invalid_message_ids_without_panicking() {
    let message =
      PublishMessage::new("events.messages", Bytes::new()).expect("publish message must be valid");
