...
mod tests {
  ...
  // Публичный JSON-формат результата операции. Внутренний формат Redis ledger
  // хранит отдельные поля, из которых адаптер собирает этот тип. Эталоны лежат в `snapshots/` рядом с этим файлом: любое
