...
pub struct BroadcastMessage {
  ...
  #[validate(length(min = 1, code = "empty_message_id", message = "Id must not be empty"))]
...
fn validate_message_data(data: &Value) -> Result<(), ValidationError> {
  match data {
    ...
    Value::Bool(_) | Value::Number(_) => {
      let mut error = ValidationError::new("unsupported_data_type");

      error.message = Some("message data must be a string, array, object, or null".into());
...
fn validate_encoding(encoding: &str) -> Result<(), ValidationError> {
  if encoding.is_empty() || encoding.split('/').all(is_valid_encoding_segment) {
    return Ok(());
  }

  let mut error = ValidationError::new("invalid_encoding");

  error.message = Some("encoding contains an invalid transformation chain".into());
...
fn is_valid_encoding_segment(segment: &str) -> bool {
  let (transform, parameter) = match segment.split_once('+') {
    Some((transform, parameter)) => (transform, Some(parameter)),
    None => (segment, None),
  };
